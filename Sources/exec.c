...
/*   Updated: 2022/12/24 17:55:36 by samirqatim       ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int exec_pipe(t_execution *exec_struct, t_command *command, t_context context)
{
    ...
    child += execCommandOfNode(exec_struct, pipe_struct.left_node, pipe_struct.left_context);
    ...
    if (command->next->next && !check_type_of_command(command->next->next->command[0]))
        close(p[STDOUT_FILENO]);
    pipe_struct.right_node = command->next;
    child += execCommandOfNode(exec_struct, pipe_struct.right_node, pipe_struct.right_context);
